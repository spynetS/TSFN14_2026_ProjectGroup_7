...
We have a terraform for azure which can be deployed using
#+BEGIN_SRC bash
cd terraform
terraform init
terraform apply
...
To get the cridentials for the acr (github secrets)
#+begin_src bash
az acr credential show --name fitnessduelacr
#+end_src


Build and deploy the image to the container registry (auto with cicd)
#+BEGIN_SRC bash
cd ../backend
docker build -t <azure_login>/fitnessduel-backend:latest
docker push
#+end_SRC

And to deploy the manifests
(Remeber to login with azure cli)

#+begin_SRC bash
cd ../manifests
kubectl apply -f
# to create a secret for the session
kubectl create secret generic backend-secrets \
  --from-literal=SESSION_SECRET="your-super-secret-salt"
#+end_SRC
